    ...
    void Scene::load(std::string file_path) {
        ...
        for (int i = 0; i < gltf_model.meshes.size(); i++) {
            ...
            for (int j = 0; j < gltf_mesh.primitives.size(); j++) {
                ...
                const tinygltf::Accessor &position_accessor = gltf_model.accessors[gltf_primitive.attributes.find(
                        "POSITION")->second];
                const tinygltf::BufferView &position_buffer_view = gltf_model.bufferViews[position_accessor.bufferView];
                position_buffer = reinterpret_cast<const float *>(&(gltf_model.buffers[position_buffer_view.buffer].data[
                ...
                if (gltf_primitive.attributes.find("NORMAL") != gltf_primitive.attributes.end()) {
                    const tinygltf::Accessor &normal_accessor = gltf_model.accessors[gltf_primitive.attributes.find(
                            "NORMAL")->second];
                    const tinygltf::BufferView &normal_buffer_view = gltf_model.bufferViews[normal_accessor.bufferView];
                    normal_buffer = reinterpret_cast<const float *>(&(gltf_model.buffers[normal_buffer_view.buffer].data[
                            normal_accessor.byteOffset + normal_buffer_view.byteOffset]));
                }

                if (gltf_primitive.attributes.find("TEXCOORD_0") != gltf_primitive.attributes.end()) {
                    const tinygltf::Accessor &uv_accessor = gltf_model.accessors[gltf_primitive.attributes.find(
                            "TEXCOORD_0")->second];
                    const tinygltf::BufferView &uv_buffer_view = gltf_model.bufferViews[uv_accessor.bufferView];
                    uv_buffer = reinterpret_cast<const float *>(&(gltf_model.buffers[uv_buffer_view.buffer].data[
                            uv_accessor.byteOffset + uv_buffer_view.byteOffset]));
                }
                ...
                switch (index_accessor.componentType) {
                    case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT: {
                        ...
                        memcpy(buf, &index_buffer.data[index_accessor.byteOffset + index_buffer_view.byteOffset],
                    ...
                    default:
                        std::cerr << "Index component type " << index_accessor.componentType << " not supported!"
                                  << std::endl;
                }
