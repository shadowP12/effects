...
void Mesh::initialize()
{
    ...
    if (mData->getMeshDesc()->hasMeshAttribute(MAS_POSITION))
    {
        ...
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, mData->getVertexStrideSize(), (GLvoid*)mData->getMeshDesc()->getMeshAttributeOffset(MAS_POSITION));
    }

    if (mData->getMeshDesc()->hasMeshAttribute(MAS_TEXCOORD))
    {
        glEnableVertexAttribArray(1);
        glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, mData->getVertexStrideSize(), (GLvoid*)mData->getMeshDesc()->getMeshAttributeOffset(MAS_TEXCOORD));
    }
