 ...
 HOW A SIMPLE RENDERING FUNCTION MAY LOOK LIKE:
    ...
    void void MyImGuiRenderFunction(ImDrawData* draw_data)
    {
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          const ImDrawVert* vtx_buffer = cmd_list->VtxBuffer.Datas;  // vertex buffer generated by Dear ImGui
          const ImDrawIdx* idx_buffer = cmd_list->IdxBuffer.Datas;   // index buffer generated by Dear ImGui
    ...
    for (ItemCount i = 0; i < item_count; i++)
    {
        ...
        for (CFIndex j = 0, nj = CFArrayGetCount(flavor_type_array); j < nj; j++)
        {
            ...
            if (PasteboardCopyItemFlavorData(main_clipboard, item_id, CFSTR("public.utf8-plain-text"), &cf_data) == noErr)
            {
                ...
                CFDataGetBytes(cf_data, CFRangeMake(0, length), (UInt8*)clipboard_text.Datas);
                clipboard_text[length] = 0;
                CFRelease(cf_data);
                return clipboard_text.Datas;
            }
        }
    }
    return NULL;
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
...
#if 0
    if (ImGui::TreeNode("Tables", "Tables (%d)", g.Tables.Datas.Size))
    {
        ImGui::TreePop();
    }
#endif
    ...
    if (ImGui::TreeNode("Internal state"))
    {
        ...
        ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d", g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer, g.HoveredIdAllowOverlap); // Datas is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
